...
object SelfEmploymentStatementResource extends BaseResource {
  ...
  def finaliseEndOfPeriodStatement(nino: Nino, id: SourceId, year: TaxYear): Action[JsValue] =
    APIAction(nino, SourceType.SelfEmployments).async(parse.json) { implicit request =>
      ...
      val declaration = request.body.as[Declaration]

      def isFinalised(declaration: Declaration): Option[Errors.Error] = declaration match {
        case _ if !declaration.finalised => Some(Errors.NotFinalisedDeclaration)
        case _                           => None
      }
      ...
      for {
        result <- authorise[Declaration, EmptyResponse](declaration, isFinalised) { _ =>
